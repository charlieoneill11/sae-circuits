...
for i in range(1, 4):
    ...
    resid_streams = torch.load(f"../data/{task}/easy_negs/resid_heads_mean_{i}.pt").to(device)
    head_labels = torch.load(f'../data/{task}/easy_negs/labels_heads_mean.pt')
    ground_truth = torch.load(f'../data/{task}/easy_negs/ground_truth.pt')
    ...
    overall_results[i] = roc_results

print(overall_results)
# Save as JSON to output
import json

with open(f"../output/data/{task}/roc_results_easy_negs.json", 'w') as f:
