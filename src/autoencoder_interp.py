...
def loss_fn(
    ...
    if alpha_ > 0:
        ...
        learned_activations_pos = einops.rearrange(
            learned_activations_pos, "n s d -> d s n"
        )
        learned_activations_neg = einops.rearrange(
            learned_activations_neg, "n s d -> d s n"
        )
        pos_sim_loss = calculate_similarity_loss(
            learned_activations_pos, learned_activations_neg, verbose=verbose
        )
    else:
    ...
    # combine
    return (
        recon_loss + (lambda_ * sparsity_loss) + (alpha_ * pos_sim_loss),
        recon_loss,
        sparsity_loss,
        pos_sim_loss,
    )
...
def train(
    ...
    for epoch in range(n_epochs):
        ...
        loss, recon_loss, sparsity_loss, pos_sim_loss = loss_fn(
            decoded_activations,
            learned_activations,
            train_streams,
            train_labels,
        ...
        if epoch % (n_epochs // 10) == 0:
            ...
            with torch.no_grad():
                ...
                eval_loss, _, _, eval_pos_sim_loss = loss_fn(
...
def gen_softmaxed_unique_to_pos(
    all_indices, ground_truth_array, head_labels, normalise=False, num_codes=250
):
...
def pairwise_cosine_similarities(pos_examples, neg_examples, eps=1e-12):
    ...
    pos_examples_perm = pos_examples.permute(
        1, 2, 0
    )  # Change to shape (S, N, D) for batch processing
    neg_examples_perm = neg_examples.permute(
        1, 0, 2
    )  # Change to shape (S, D, M) for correct dot product
    ...
    magnitude_p = torch.sqrt(
        torch.einsum("snd,snd->sn", pos_examples_perm, pos_examples_perm) + eps
    ).unsqueeze(-1)
    magnitude_n = torch.sqrt(
        torch.einsum("sdm,sdm->sm", neg_examples_perm, neg_examples_perm) + eps
    ).unsqueeze(-2)
    ...
    average_cosine_similarities_per_position = torch.mean(
        cosine_similarities, dim=(1, 2)
    )
...
for i in range(num_trials):
    ...
    if task_type == "node":
        ...
        print(f"\n\nNormalise: {normalise}")
        y_true, y_pred, fpr, tpr, node_roc_auc, f1, thresholds = (
            gen_softmaxed_unique_to_pos(
                all_indices, ground_truth_array, head_labels, normalise=normalise
            )
        )
            ...
            for e in range(all_indices.shape[0]):  # For each example
                for h1 in range(n_heads):  # For each head
                    ...
                    for h2 in range(n_heads):  # For each other head
                        if (
                            h1 != h2
                        ):  # Skip counting co-occurrence of a head with itself
                            c2 = all_indices[e, h2]  # Code in head h2
                            # Increment co-occurrence count for (h1, h2)
                            co_occurrence_matrix[h1, h2, c1, c2] += 1
                ...
                for h2 in range(n_heads):
                    if h1 != h2:  # Skip self co-occurrences
                        ...
                        if normalise:
                            ...
                            if total_co_occurrences > 0:  # Avoid division by zero
                                unique_count_normalized = (
                                    np.sum(unique) / total_co_occurrences
                                )
                            else:
                                unique_count_normalized = 0
                            # Set normalized unique counts for this head pair
                            unique_co_occurrence_counts[h1, h2] = (
                                unique_count_normalized
                            )
                        else:
                            # Count unique co-occurrences
        ...
        unique_co_occurrence_counts = unique_co_occurrences(
            positive_co_occurrence_matrix,
            negative_co_occurrence_matrix,
            normalise=normalise,
        )
        ...
        sorted_indices = np.unravel_index(
            sorted_indices, unique_co_occurrence_counts.shape
        )
        ...
        print(
            f"Best ROC AUC across k: {np.max(roc_auc_values):.4f} (at k={k_values[np.argmax(roc_auc_values)]})"
        )
