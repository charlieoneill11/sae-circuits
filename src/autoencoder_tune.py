...
def objective(trial):
    ...
    num_unique = trial.suggest_int('num_unique', 128, 2048)  # Example range, adjust based on your constraints
    lambda_ = trial.suggest_float('lambda_', 1e-2, 1e-1, log=True)  # Log scale search
    ...
    # Training routine
    try:
        model = train(model, n_epochs, optimizer, train_streams, eval_streams, lambda_=lambda_, alpha_=0.0, verbose=False)
        model = model.to('cpu')
        # Evaluation step to get the ROC AUC score
        model.eval()
        resid_streams_cpu = resid_streams.to('cpu')
        learned_activations = model(resid_streams_cpu)[0].detach().cpu().numpy()
        all_indices = np.argmax(learned_activations, axis=2)
        y_true, y_pred, fpr, tpr, roc_auc, f1, thresholds = gen_softmaxed_unique_to_pos(all_indices, ground_truth_array, head_labels, normalise=normalise)
        best_roc_auc = torch.load(f"../models/{task}/best_roc_auc.pt")
        if roc_auc > best_roc_auc:
            print(f"New best ROC AUC: {roc_auc} (previous best: {best_roc_auc}). Saving model...")
            best_roc_auc = roc_auc
            # Save the best ROC AUC
            torch.save(best_roc_auc, f"../models/{task}/best_roc_auc.pt")
            # Save model
            savepath = f"../models/{task}/sparse_autoencoder.pt"
            save_dict = {
                'model': model.state_dict(),
                'node_best_roc_auc': best_roc_auc,
                'node_best_lambda': lambda_,
                'node_best_num_unique': num_unique
            }
            torch.save(save_dict, savepath)
        return roc_auc  # Objective: Maximize ROC AUC
    except Exception as e:
        print(f"Failed training with exception: {e}")
        return 0

# Best ROC AUC
best_roc_auc = 0.0
# Save to torch file
savepath = f"../models/{task}/best_roc_auc.pt"
torch.save(best_roc_auc, savepath)
...
# Save the study
savepath = f"../output/{task}/"  # Adjust this to your preferred save path
joblib.dump(study, f"{savepath}optuna_study.pkl")  # save study